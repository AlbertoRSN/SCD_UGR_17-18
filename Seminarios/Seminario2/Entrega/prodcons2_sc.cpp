...
const int num_hebras_productoras = 10;
const int num_hebras_consumidoras = 10;
...
int buffer[num_hebras_productoras];
int tam_buffer = 0;
...
//Recibe numero de hebra que lo ejecuta.
int producir_dato(int num_hebra)
{
   static int contador = 0 ;
   static int num_producido = 0;
   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "producido: " << num_producido << endl << flush ;
   tam_buffer++;
   cout<< "numero de elementos producidos: " << tam_buffer << endl << flush;
   mtx.unlock();
   cont_prod[contador]++ ;
   contador++;

   for(int i = num_hebra; i<num_hebra+contador; i++){
      num_producido++;
      buffer[tam_buffer] = num_producido;
   }
...
void funcion_hebra_productora( ProdCons1SC * monitor, int num )
{
   for( unsigned i = 0 ; i < num_items ; i++ ){
...
int main()
{
   ...
   for(int j=0; j<num_hebras_consumidoras; j++){
